...
export default function SearchView({ consultations, language }: { consultations: Consultation[], language: Language }) {
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
        // 1. Local Vector Search (Python Backend)
        ...
        }).sort((a,b) => b.score - a.score).filter(x => x.score > 0.3);

        let found = scores.map(s => consultations.find(c => c.id === s.id)!);

        // Fallback to Groq text search if empty
        if (found.length === 0) {
